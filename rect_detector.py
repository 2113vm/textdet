...
class RectDetector:
    ...
    def linear_line_matrix(self, p0, p1):
    ...
    def rotate_to_pretty(self, points):
        ...
        w = self.find_width(points)
        h = self.find_hieght(points)
    ...
    def detect_pretty_w_h_to_zone(self, w, h, coef=4.6):
    ...
    def get_cv_zonesRGB(self, img, rects, gw=0, gh=0, coef=4.6):
        ...
        for rect in rects:
            ...
            if (gw == 0 or gh == 0):
                w, h = self.detect_pretty_w_h_to_zone(w, h, coef)
            else:
                w, h = gw, gh
    ...
    def make_ugly_points(self, points):
    ...
    def find_distances(self, points):
        ...
        for i in range(cnt):
            ...
            distanses.append({"d": self.distance(points[p0][0], points[p1][0]), "p0": p0, "p1": p1,
                              "matrix": self.linear_line_matrix(points[p0][0], points[p1][0]),
                              "coef": self.fline(points[p0][0], points[p1][0])})
        return distanses

    def clac_rect_lines(self, distanses):
        ''' Sort lines to length '''
        return sorted(distanses, key=lambda x: x["d"])

    def filter_interested_lines(self, interestedLines, minElements, thresholdPercentage):
    ...
    def linear_line_matrix_cross_point(self, p0, p1, point):
        arr = self.linear_line_matrix(p0, p1)
        x = point[0]
        y = point[1]
        arr[2] = arr[0] * x + arr[1] * y
        return arr

    def detect_similar_lines(self, C, targetLine, significantAlignPercentage=0.3):
        arr = []
    ...
    def detect_alignment_lines(self, targetLines, A, B):
        arrA0 = self.detect_similar_lines(A, targetLines[0])
        arrB0 = self.detect_similar_lines(B, targetLines[1])
        arrA1 = self.detect_similar_lines(A, targetLines[2])
        arrB1 = self.detect_similar_lines(B, targetLines[3])
        return np.array([arrA0, arrB0, arrA1, arrB1])

    def make_aligment_key_points(self, line, arr):
        commonPoint = -1
        startPoint = -1
        endPoint = -1
        subline = arr[0]
    ...
    def make_aligment_matrix(self, line, arr, points):
        if (len(arr) == 0):
            return line["matrix"]
        else:
            pObj = self.make_aligment_key_points(line, arr)
            return self.linear_line_matrix_cross_point(points[pObj["sPoint"]][0], points[pObj["ePoint"]][0],
                                                       points[pObj["cPoint"]][0])

    def make_target_points2(self, targetLines, targetAlignmentLines, points):

        line0matrix = self.make_aligment_matrix(targetLines[0], targetAlignmentLines[0], points)
        line1matrix = self.make_aligment_matrix(targetLines[1], targetAlignmentLines[1], points)
        line2matrix = self.make_aligment_matrix(targetLines[2], targetAlignmentLines[2], points)
        line3matrix = self.make_aligment_matrix(targetLines[3], targetAlignmentLines[3], points)
    ...
    def gKMeansMajorLines(self, interestedLines, minElements=5):
        ...
        interestedLinesFiltered = self.filter_interested_lines(interestedLines, 4, 0.2)
    ...
    def addOffset(self, targetPoints, offsetHorisontal, offsetVertical):
        distanses = self.find_distances(self.make_ugly_points(targetPoints))
        points = []
        cnt = len(distanses)
        ...
        for i in range(cnt):
            ...
            points.append(
                self.detectIntersectionNormDD(distanses[iPrev]['matrix'], distanses[iNext]['matrix'], offset1, offset2))
        ...
        if d0 < d1:
            ...
            if abs(angle0) > abs(angle1):
                if (distanses[2]["d"] < distanses[0]["d"]):
                    points[3] = self.detectIntersectionParalelLine(distanses[2], distanses[1],
                                                                   points[distanses[0]["p0"]])
                else:
                    points[0] = self.detectIntersectionParalelLine(distanses[0], distanses[1],
                                                                   points[distanses[2]["p1"]])
            else:
                if (distanses[2]["d"] < distanses[0]["d"]):
                    points[2] = self.detectIntersectionParalelLine(distanses[2], distanses[3],
                                                                   points[distanses[0]["p1"]])
                else:
                    points[1] = self.detectIntersectionParalelLine(distanses[0], distanses[3],
                                                                   points[distanses[2]["p0"]])
        else:
            angle1 = distanses[1]["coef"][3] - distanses[2]["coef"][3]
            if abs(angle0) > abs(angle1):
                if (distanses[3]["d"] < distanses[1]["d"]):
                    points[3] = self.detectIntersectionParalelLine(distanses[3], distanses[0],
                                                                   points[distanses[1]["p1"]])
                else:
                    points[2] = self.detectIntersectionParalelLine(distanses[1], distanses[0],
                                                                   points[distanses[3]["p0"]])
            else:
                if (distanses[3]["d"] < distanses[1]["d"]):
                    points[0] = self.detectIntersectionParalelLine(distanses[3], distanses[2],
                                                                   points[distanses[1]["p0"]])
                else:
                    points[1] = self.detectIntersectionParalelLine(distanses[1], distanses[2],
                                                                   points[distanses[3]["p1"]])
        return points

    def fixRectangle(self, points, fixRectangleAngle=3):
        distanses = self.find_distances(self.make_ugly_points(points))
    ...
    def unique_points(self, arrPoints):
        arrPointsNew = []
    ...
    def detect(self, images, outboundWidthOffset=3, outboundHeightOffset=0, fixRectangleAngle=3, fixGeometry=1):
        ...
        for image in images:
            ...
            print(1)
            res = []
            arrPoints = self.detect_rect(image)
            arrPoints = self.unique_points(arrPoints)
            print(arrPoints)

            for points in arrPoints:
                distanses = self.find_distances(points)
                interestedLines = self.clac_rect_lines(distanses)
                ...
                if fixGeometry:
                    targetAlignmentLines = self.detect_alignment_lines(targetLines, A, B)
                    targetPoints = self.make_target_points2(targetLines, targetAlignmentLines, arrPoints[0])
                else:
...
if __name__ == '__main__':
    ...
    arrPoints = rectDetector.detect(mask_of_image)
    zones = rectDetector.get_cv_zonesBGR(image, arrPoints)
